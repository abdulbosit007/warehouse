...
function FilterModal({ categories, locations, filters, onApply, onClose }) {
  ...
  const activeCount = [localFilters.category_id, localFilters.stockStatus, localFilters.location_id].filter(Boolean).length;
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm px-4">
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-lg bg-white rounded-t-3xl sm:rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        <div className="bg-gradient-to-r from-blue-600 to-cyan-600 px-6 py-4 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {activeCount > 0 && <p className="text-sm text-blue-100">{activeCount} active</p>}
            </div>
          </div>
        ...
        <div className="p-6 space-y-5 overflow-y-auto flex-1">
          ...
          {/* Location */}
          <div>
            <label className="block text-xs font-semibold text-neutral-700 uppercase tracking-wider mb-2">Warehouse</label>
            <CustomSelect
              value={localFilters.location_id || ""}
              onChange={(val) => setLocalFilters({ ...localFilters, location_id: val })}
              placeholder="All Warehouses"
              color="blue"
              options={[
                { value: "", label: "All Warehouses" },
                ...locations.map((loc) => ({ value: loc.id, label: loc.location_name || loc.name }))
              ]}
            />
          </div>
        </div>
        ...
        <div className="border-t border-neutral-100 px-6 py-4 bg-neutral-50 flex items-center justify-between gap-3 shrink-0">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={handleApply} className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-blue-600 to-cyan-600 px-4 py-2 text-sm font-semibold text-white shadow-lg">
...
export default function WarehouseHome() {
  const { loading: authLoading, error: authError, roleBase, locationName } = useCurrentUser();
  ...
  const [filters, setFilters] = useState({ category_id: "", stockStatus: "", location_id: "" });
  ...
  const quantityMap = useMemo(() => {
    ...
    for (const item of productList) {
      ...
      locMap.set(item.location_id, (locMap.get(item.location_id) || 0) + (item.quantity || 0));
    }
    return map;
  }, [productList]);
  ...
  const getQtyAt = (productId, locationId) => quantityMap.get(productId)?.get(locationId) || 0;
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (filters.stockStatus) {
      result = result.filter((p) => {
        const qty = filters.location_id ? getQtyAt(p.id, filters.location_id) : getTotalQty(p.id);
  ...
  const activeFilterCount = [filters.category_id, filters.stockStatus, filters.location_id].filter(Boolean).length;

  // Stats
  const totalStock = productList.reduce((sum, item) => sum + (item.quantity || 0), 0);
  const lowStockCount = productsWithStock.filter((p) => { const q = getTotalQty(p.id); return q > 0 && q <= 10; }).length;
      ...
      <div className="p-6">
        <div className="rounded-2xl border border-red-200 bg-gradient-to-r from-red-50 to-rose-50 p-6 text-red-700">
          <div className="flex items-center gap-3">
            ...
            <span className="font-medium">{authError || "Warehouse access only."}</span>
          </div>
        </div>
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-neutral-500">{locationName || "All warehouses"} • {productsWithStock.length.toLocaleString()} products</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="rounded-2xl p-4 text-left" style={{ background: "linear-gradient(135deg, #dbeafe 0%, #bfdbfe 100%)" }}>
          <div className="flex items-start justify-between">
            <div>
              <p className="text-xs font-medium text-blue-600 uppercase tracking-wider">Locations</p>
              <p className="mt-1 text-3xl font-bold text-neutral-900">{locations.length}</p>
            </div>
            <div className="p-2 rounded-xl text-blue-600 bg-white/50"><Warehouse className="w-5 h-5" /></div>
          </div>
        </div>
        ...
        <div className="rounded-2xl p-4 text-left" style={{ background: "linear-gradient(135deg, #d1fae5 0%, #a7f3d0 100%)" }}>
          <div className="flex items-start justify-between">
            <div>
              <p className="text-xs font-medium text-emerald-600 uppercase tracking-wider">Total Stock</p>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <button
          ...
          className={`inline-flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm font-medium transition-all ${
            activeFilterCount > 0 ? "border-blue-300 bg-blue-50 text-blue-700" : "border-neutral-200 bg-white text-neutral-700 hover:bg-neutral-50"
          }`}
        >
          <Filter className="w-4 h-4" />Filters
          {activeFilterCount > 0 && <span className="bg-blue-600 text-white text-xs font-bold px-1.5 py-0.5 rounded-full">{activeFilterCount}</span>}
        </button>
      ...
      {activeFilterCount > 0 && (
        <div className="flex flex-wrap gap-2">
          ...
          {filters.location_id && (
            <span className="inline-flex items-center gap-1.5 rounded-full bg-blue-50 border border-blue-200 px-3 py-1 text-xs font-medium text-blue-700">
              {locations.find((l) => l.id === filters.location_id)?.location_name || "Location"}
              <button onClick={() => setFilters({ ...filters, location_id: "" })}><X className="w-3 h-3" /></button>
            </span>
          )}
          <button onClick={() => setFilters({ category_id: "", stockStatus: "", location_id: "" })} className="text-xs text-neutral-500 hover:text-neutral-700">Clear all</button>
        </div>
      )}
      ...
      <div className="rounded-2xl border border-neutral-200 bg-white shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-16 text-neutral-500">
            ...
            {activeFilterCount > 0 && <button onClick={() => setFilters({ category_id: "", stockStatus: "", location_id: "" })} className="mt-2 text-sm text-blue-600">Clear filters</button>}
          </div>
        ) : (
            ...
            <table className="min-w-full">
              <thead>
                <tr className="bg-gradient-to-r from-blue-600 to-cyan-600">
                  ...
                  <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-wider text-white bg-blue-700">Total</th>
                  {locations.map((loc) => (
                    <th key={loc.id} className="px-3 py-3 text-center text-xs font-semibold uppercase tracking-wider text-white bg-blue-600" title={loc.location_name || loc.name}>
                      <div className="flex items-center justify-center gap-1">
                        <Warehouse className="w-3 h-3" />
                        <span className="truncate max-w-[80px]">{(loc.location_name || loc.name || "").slice(0, 10)}</span>
                      </div>
                    </th>
                  ))}
                </tr>
                    ...
                    <tr key={product.id} className={`hover:bg-neutral-50 transition-colors ${isLowStock ? "bg-amber-50" : ""}`}>
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className={`inline-flex items-center justify-center min-w-[40px] px-2 py-0.5 rounded-full text-xs font-bold ${
                          isLowStock ? "bg-amber-100 text-amber-700" : totalQty > 0 ? "bg-blue-100 text-blue-700" : "bg-neutral-100 text-neutral-400"
                        }`}>{totalQty}</span>
                      </td>
                      {locations.map((loc) => {
                        ...
                        return (
                          <td key={loc.id} className="px-3 py-3 text-center">
                            <span className={`inline-flex items-center justify-center min-w-[32px] px-1.5 py-0.5 rounded text-xs font-medium ${qty > 0 ? "bg-blue-50 text-blue-700" : "text-neutral-300"}`}>
                              {qty || "—"}
                            </span>
                          </td>
                        );
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1">
              {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                ...
                return (
                  <button key={pageNum} onClick={() => setPage(pageNum)} className={`min-w-[36px] h-9 rounded-lg text-sm font-medium ${page === pageNum ? "bg-blue-600 text-white" : "bg-white border border-neutral-200 text-neutral-700 hover:bg-neutral-50"}`}>
                    {pageNum}
                  </button>
                );
              })}

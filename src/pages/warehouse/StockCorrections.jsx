...
export default function WarehouseStockCorrections() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (err) {
        ...
        setLocationError(err.message || "Could not load warehouse locations.");
      } else if (!data || data.length === 0) {
        setAllWarehouses([]);
        setWarehouseLocation(null);
        setLocationError("No warehouse locations found in the database.");
      } else {
  ...
  async function handleCreateCorrection(e) {
    ...
    if (!warehouseLocation) {
      setSaveError("Warehouse location not loaded.");
      return;
    }
    ...
    try {
      ...
      if (insertErr) {
        if (
          ...
          throw new Error(
            "There is already a pending correction for this product. Wait for owner decision."
          );
        }
        throw insertErr;
      }
  ...
  const filtered = useMemo(() => {
    return corrections.filter((c) => {
      ...
      return (
        productName.includes(q) || sku.includes(q) || requesterName.includes(q)
      );
    });
  ...
  const locationLabel =
    warehouseLocation?.location_name || warehouseLocation?.name || "Warehouse";
